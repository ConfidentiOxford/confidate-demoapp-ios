...
class BertTokenizer {
    ...
    init(maxLen: Int) {
        print("Bundle.main.url: ", Bundle.main)
    ...
    func tokenize(text: String) -> [String] {
        ...
        let tokensNumber = tokens.count
        print("number of tokens before: ", tokensNumber)
        if tokensNumber > maxLen - 2 {
            tokens = tokens.dropLast(tokensNumber - maxLen - 2)
        } else if (tokensNumber < maxLen - 2) {
            tokens.append(
                contentsOf: Array(repeating: "[PAD]", count: maxLen - 2 - tokensNumber)
            )
        }
        tokens = ["[CLS]"] + tokens + ["[SEP]"]
        print("number of tokens after: ", tokens.count)
        return tokens
    }
